...
def generar_slots_tiempo(hora_inicio: time, hora_fin: time, duracion_minutos: int = 60) -> List[time]:
    """
...
async def obtener_horarios_disponibles_proveedor(
    ...
    duracion_minutos: int = Query(60, ge=15, le=480, description="Duración de cada slot en minutos"),
    ...
    try:
        ...
        if excepcion_dia and excepcion_dia.tipo == 'cerrado':
            return []  # Cerrado este día
        
        # 3. Determinar horario efectivo
        if excepcion_dia and excepcion_dia.tipo == 'horario_especial':
        ...
        reservas_query = select(ReservaModel).where(
            and_(
                ...
                ReservaModel.estado.in_(['confirmada', 'pendiente'])
            )
        )
        ...
        raise HTTPException(
            ...
            detail="Error al obtener horarios disponibles."
        )
...
async def obtener_horarios_disponibles_servicio(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Error al obtener horarios disponibles para el servicio."
        )
...
async def obtener_horarios_rango_fechas(
    ...
    try:
        ...
        if (fecha_fin - fecha_inicio).days > 30:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="El rango de fechas no puede ser mayor a 30 días."
            )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Error al obtener horarios para el rango de fechas."
        )
