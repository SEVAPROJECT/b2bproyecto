...
const AvailabilityManager: React.FC<AvailabilityManagerProps> = ({ servicioId, servicioNombre }) => {
    ...
    const loadDisponibilidades = async () => {
        try {
            ...
            const response = await fetch(buildApiUrl(`/disponibilidades/servicio/${servicioId}`), {
                headers: {
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            ...
            const response = await fetch(buildApiUrl(`/disponibilidades/${id}`), {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${user?.accessToken}`,
                    'Content-Type': 'application/json',
    ...
    // Función auxiliar para renderizar la lista de disponibilidades
    const renderDisponibilidadesList = () => {
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                onChange={(e) => setFormData(prev => ({ ...prev, precio_adicional: Number.parseFloat(e.target.value) || 0 }))}
                                className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                            />
                        </div>
